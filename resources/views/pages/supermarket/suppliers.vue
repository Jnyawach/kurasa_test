            ...
            <div class="bg-white shadow rounded-xl">
                <div class="flex justify-between p-3 py-5">
                    ...
                    <div class="flex gap-1">
                        ...
                        <div>
                            <button @click="newSupplier=true" class="font-bold bg-sky-600 text-white py-2 px-5 rounded-lg flex items-center gap-1">
                ...
                <div class="divide-y" v-if="grid===1">
                    <div class=" p-2 py-5" v-for="supplier in supermarket.data.suppliers" :key="supplier.id">
                        <div class="flex gap-5 justify-between">
                            <div>
                                <p class="font-bold text-lg">{{supplier.name}}</p>
                                <div>
                                    <p><span class="text-sky-700 mr-2 font-medium">Cellphone:</span>{{supplier.phone}}</p>
                                    <p><span class="text-sky-700 mr-2 font-medium">Location:</span>{{supplier.town}}, {{supplier.country}}</p>
                                </div>
                            </div>
                            <div class="flex gap-2 items-center">
                                ...
                                <div>
                                    <Link method="DELETE" as="button" :href="'/supplier/'+supplier.id" class="text-red-600 m-2 font-medium hover:text-red-700">
                                        Delete
                                    </Link>
                                </div>
                ...
                <div class="grid grid-cols-3 p-3 gap-3" v-if="grid===2">
                    <div class="shadow border rounded-lg p-3" v-for="supplier in supermarket.data.suppliers" :key="supplier.id">
                        <div class="flex items-center justify-between">
                            <h2 class="font-bold text-lg">{{supplier.name}}</h2>

                        </div>
                        <div>
                            <p><span class="text-sky-700 mr-2 font-medium">Phone:</span>{{supplier.phone}}</p>
                            <p><span class="text-sky-700 mr-2 font-medium">Location:</span>{{supplier.town}} {{supplier.country}}</p>
