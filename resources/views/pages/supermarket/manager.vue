    ...
    <main-layout>
        <template #breadcrumb>
            <ul class="my-2 flex divide-x divide-sky-600">
                ...
                <li class="px-2">
                    <Link :title="supermarket.name" :href="'/supermarket/'+supermarket.id" class="hover:text-sky-600">{{supermarket.name}}</Link>
                </li>
                <li class="px-2">
                    <Link :title="manager.data.name" href="#" class="hover:text-sky-600">{{manager.data.name}}</Link>
                </li>
                <li class="px-2">
                    <Link title="Employees" href="#" class="hover:text-sky-600">Employees</Link>
                </li>
            </ul>
        </template>
        <div>
            ...
            <div class="bg-white shadow rounded-xl">
                <div class="flex justify-between p-3 py-5">
                    <div>
                        <h2 class="text-lg font-bold">{{supermarket.name}}</h2>
                        <h6 class="font-bold">Manager: <span class="text-sky-600">{{manager.data.name}}</span></h6>
                    ...
                    <div class="flex gap-1">
                        ...
                        <div>
                            <button @click="newEmployee=true" class="font-bold bg-sky-600 text-white py-2 px-5 rounded-lg flex items-center gap-1">
                ...
                <div class="divide-y" v-if="grid===1">
                    <div class=" p-2 py-5" v-for="employee in manager.data.employees" :key="employee.id">
                        <div class="flex gap-5 justify-between">
                            <div>
                                <p class="font-bold text-lg">{{employee.name}}</p>
                                <div>

                                    <p><span class="text-sky-700 mr-2 font-medium">Gender:</span>{{employee.gender}}</p>
                                    <p><span class="text-sky-700 mr-2 font-medium">Type:</span>{{employee.type}}</p>
                                </div>
                            </div>
                            <div class="flex gap-2 items-center">
                                ...
                                <div>
                                    <Link method="DELETE" as="button" :href="'/employee/'+employee.id" class="text-red-600 m-2 font-medium hover:text-red-700">
                                        Delete
                                    </Link>
                                </div>
                ...
                <div class="grid grid-cols-2 p-3 gap-3" v-if="grid===2">
                    <div class="shadow border rounded-lg p-3" v-for="employee in manager.data.employees" :key="employee.id">
        ...
        <!--new manager modal-->
        <new-employee :show="newEmployee" @close="newEmployee=false" :manager="manager.data"></new-employee>
    </main-layout>
</template>
...
import NewEmployee from "@/views/components/new-employee.vue";
import EditEmployee from "@/views/components/edit-employee.vue";
defineProps({
    supermarket:Object,
    manager:Object
})
